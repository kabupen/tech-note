...
categories: ["統計"]
tags: ["変数変換", "ヤコビアン"]
description: "一変量の置換積分から出発し、確率変数の変数変換を一対一対応・単調性の観点から整理します。さらに、多変量への拡張としてヤコビアンがどのように微小体積を「拡大・縮小」する係数になるのか、微分の向き・並べ方の覚え方まで含めて解説します。"
---

# 変数変換

何らかの変換により、$X$ が $Y$ に、$(X, Y)$ が $(S, T)$ に変換される問題を想定します。
...
と求めることができます^[**変換式が単調** であるということは改めて抑えておきましょう（$f$ についてではないです）。]。
