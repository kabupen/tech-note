---
title: "標本平均と標本分散"
pubDate: 2024-10-10T23:23:12+09:00
categories: ["統計"]
tags: ["標本平均", "標本分散"]
description: "正規母集団から得られる標本平均と不偏分散がどのような分布に従うのかを整理し、そこから t 分布および F 分布がどのように導かれるかを体系的にまとめます。正規分布の再生性・カイ二乗分布・t 分布・F 分布の関係を、式変形の流れとともに理解します。"
---
...
を用いた標本平均と不偏分散^[標本分散は不偏性を持っていないため、不偏性を持っている標本分散として不偏分散を用います。] が従う確率分布（標本分布）について考えてみます。
...
のように正規分布に従う確率分布 $X_i$ の和であるため、正規分布の再生性より $\bar{X}$ も正規分布に従うことが分かります^[もしくはモーメント母関数を $X$ と $\bar{X}$ について比較をすれば同様のことが導けますが、毎回計算しているのも面倒なので、再生性については覚えておいた方が良い性質です。]。あとは平均と分散を求めると、
...
です^[$X\sim N(\mu, \sigma^2)$、$\bar{X} \sim N(\mu, \sigma^2/n)$ であるので、それぞれ平均と分散とを用いて標準化すると標準正規分布に従います。]。標準正規分布に従う確率変数の二乗和がカイ二乗分布に従うことを思い出すと、
...
で定義される確率変数 $Y$ は自由度 $(m,n)$ の F 分布と呼ばれる確率分布に従います。
